<div class=" col-12 col-sm-12 col-lg-3 sidebar sidebar-left">
    <?php
    $selectStmt = $conn->prepare("SELECT * FROM bike_details_table WHERE bike_availability = '' ORDER BY RAND() LIMIT 3");
    $selectStmt->execute();
    ...
    ?>
            <div style="border: 2px solid #ccc; padding: 20px;">
                <p style="font-weight: bold; font-size: 18px; margin-bottom: 10px;">Sponsored Ads</p>
                <?php
                //Get the image
                $post_identy = htmlspecialchars($row_id['id']);
                ...
                <p style="margin-top: 10px;">
                    <br>
                    <strong>Bike name:</strong> <?php echo strtoupper(htmlspecialchars($row_id['bike_name'], ENT_QUOTES, 'UTF-8')); ?>
                    <br>
                    <strong>Year:</strong> <?php echo strtoupper(htmlspecialchars($row_id['bike_year'], ENT_QUOTES, 'UTF-8')); ?>
                    <br>
                    <strong>Engine Capacity:</strong> <?php echo strtoupper(htmlspecialchars($row_id['bike_cc'], ENT_QUOTES, 'UTF-8')); ?>cc
                    <br>
                    <strong>Mileage:</strong> <?php echo strtoupper(htmlspecialchars($row_id['bike_mileage'], ENT_QUOTES, 'UTF-8')); ?> KMs
                    <br>
                    <strong>Condition:</strong> <?php echo strtoupper(htmlspecialchars($row_id['bike_condition'], ENT_QUOTES, 'UTF-8')); ?>
                    <br>
                    <strong>Price:</strong> <?php echo number_format((float) $row_id['bike_price']); ?>
                </p>
            </div>
            <br>
    <?php }
